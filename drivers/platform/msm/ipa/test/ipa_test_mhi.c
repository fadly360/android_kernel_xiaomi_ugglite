...
static int ipa_mhi_test_initialize_driver(bool skip_start_and_conn)
{
	...
	if (wait_for_completion_timeout(&mhi_test_ready_comp, msecs_to_jiffies(10000)) == 0) {
...
static int ipa_mhi_test_suspend_aggr_open(bool force)
{
	...
	if (force) {
		if (!wait_for_completion_timeout(&mhi_test_wakeup_comp, msecs_to_jiffies(1000))) {
...
static int ipa_mhi_test_suspend_host_wakeup(void)
{
	...
	if (wait_for_completion_timeout(&mhi_test_wakeup_comp, msecs_to_jiffies(1000)) == 0) {
		IPA_UT_LOG("timeout waiting for wakeup event\n");
